...
class CertificadoAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom_urls = [
            path('importar-excel/', self.admin_site.admin_view(self.importar_excel.view),
            name='importar_excel'),
        ]
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['show_import_buttom'] = True
        return super().changelist_view(request, extra_context)
    
    def importar_excel_view(self, request):
        if request.method == 'POST':
            excel_file = request.FILES.get('excel_file')
            if excel_file and excel_file.name.endswitch(('.xlsx', '.xls')):
                try:

                    # Lê o primeiro arquivo Excel
                    df = pd.read_excel(excel_file)

                    # Verifica se todas as colunas necessárias estão presentes
                    colunas_necessarias = ['Nome do usuário', 'CPF do usuário', 'Nome do curso', 'Data início do curso', 'Data final do curso']
                ...
                messages.error(request, 'Por favor, envie um arquvo Excel válido (.xlsx ou .xls).')
                return redirect('..')
